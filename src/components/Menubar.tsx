...
const Menubar = () => {
  ...
  const handleLeftChevronClick = () => {
    ...
    console.log(shift);
    if (shift > 100) {
      setShift((prev) => Math.max(prev - 100, 0)); // Decrease shift to move items rightward
    } else {
      setShift(0);
    }
  ...
  const handleRightChevronClick = () => {
    console.log(">");
    console.log(shift);
    setShift((prev) => Math.min(prev + 100, 200)); // Increase shift to move items leftward
    console.log(shift);
    console.log(">");
  };
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (scrollContainer) {
        setShift(scrollContainer.scrollLeft); // Update shift based on scroll position
        console.log(shift);
      }
    };
    ...
    <div className="h-24 w-full flex items-center px-6 lg:px-10 xl:px-20">
      ...
      <div className="relative w-full h-full flex shrink overflow-x-hidden">
        <div
          ...
          className="w-full h-full flex shrink overflow-x-auto"
        >
          <div
            className="flex shrink-0 items-center"
            style={{
              marginLeft: `-${shift * 2}px`,
